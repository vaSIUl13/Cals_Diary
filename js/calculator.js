document.addEventListener('DOMContentLoaded', () => {
    ...
    // Кнопки
    const genderBtns = Array.from(document.querySelectorAll('.gender__option button'));
    const workoutBtns = Array.from(document.querySelectorAll('.training__options button'));

    // Вивід результату
    const caloriesDisplay = document.querySelector('.circular-chart__value');

    // Елементи БЖВ
    const proteinValue = document.querySelector('.macros__item--protein .macros__value');
    const fatValue = document.querySelector('.macros__item--fat .macros__value');
    const carbsValue = document.querySelector('.macros__item--carbs .macros__value');

    // Елементи смужок
    const proteinBarFill = document.querySelector('.macros__item--protein .macros__fill');
    const fatBarFill = document.querySelector('.macros__item--fat .macros__fill');
    const carbsBarFill = document.querySelector('.macros__item--carbs .macros__fill');
