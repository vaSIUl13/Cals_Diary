document.addEventListener('DOMContentLoaded', () => {
    ...
    // Елементи БЖВ
    const proteinGoalEl = document.querySelector('.macros__item--protein .macros__value');
    const fatGoalEl = document.querySelector('.macros__item--fat .macros__value');
    const carbsGoalEl = document.querySelector('.macros__item--carbs .macros__value');
    const proteinFill = document.querySelector('.macros__item--protein .macros__fill');
    const fatFill = document.querySelector('.macros__item--fat .macros__fill');
    const carbsFill = document.querySelector('.macros__item--carbs .macros__fill');
    ...
    // Цілі з калькулятора
    let userGoals = JSON.parse(localStorage.getItem('fitapp_goals')) || { calories: 2400, protein: 170, fat: 85, carbs: 238 };
    ...
    function getDisplayDate(dateObj) {
        ...
        if (dateObj.toDateString() === today.toDateString()) return `Сьогодні, ${formatted}`;
        if (dateObj.toDateString() === yesterday.toDateString()) return `Вчора, ${formatted}`;
        if (dateObj.toDateString() === tomorrow.toDateString()) return `Завтра, ${formatted}`;
    ...
    searchBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (data.products && data.products.length > 0) {
                ...
                if (resultsList.innerHTML === '') resultsList.innerHTML = '<li>Не знайдено продуктів з КБЖВ :(</li>';
            } else {
    ...
    function renderDiaryUI() {
        ...
        mealKeys.forEach(mealKey => {
            const mealListEl = document.querySelector(`#meal-${mealKey} .meal-card__list`);
            const mealTotalCalsEl = document.querySelector(`#meal-${mealKey} .meal-card__total`);
            ...
            if (items.length === 0) {
                mealListEl.innerHTML = '<li class="meal-card__empty-text">Ще нічого не додано</li>';
            } else {
                items.forEach(item => {
        ...
        document.querySelectorAll('.food-item__delete').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                currentDiary[mKey] = currentDiary[mKey].filter(item => item.id !== itemId);
                saveDiary();
                renderDiaryUI(); 
            });
        });
